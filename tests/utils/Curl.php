...
class Curl{
	...
	public static function post($url,array $post=[],array $options=[]){
		$defaults = array(
				...
				CURLOPT_COOKIEFILE => __DIR__.'/~cookiejar.log',
				CURLOPT_COOKIEJAR  => __DIR__.'/~cookiejar.log'
		);
		$ch = curl_init();
...
// 		}
	 ...
	 *
	 * @param string $url
	 * @param number $timeout_ms 过期时间 ms
	 * @param string $ispost
	 * @return boolean
	 */
	public static function check($url,$timeout_ms=1000,$ispost=false){
		//
		$defaults=
		[
			CURLOPT_URL 			=>$url,
